...
2. **The per-shard heaps have no bound once the frontier blocks** (Q30). The `invariant:` says
   ...
   **Hephaïstos**.
   ...
   DISCHARGED 2026-09-11 `coderoast-ipc 4d5ca7e` — no behaviour change. `CausalReorderBuffer`'s `invariant:` pair now states the bound (a heap holds only frames some live shard's watermark has not passed, so what its shard produced after the laggiest live shard's last seal: one seal interval under per-window seals), that growth past it needs a live shard that stops sealing without EOS and already reads as `frontier_blocks` rising and `buf_size` growing, and why there is no cap (a drop would be silent at the consumer; leaving frames in the ring deadlocks a producer blocked on that ring). `CausalReorderBuffer.APerShardHeapNeverHoldsMoreThanOneSealIntervalsFrames` witnesses it: shard 0 seals each window last and emits nothing else, shard 1 emits 5 data frames and its seal per window over 8 windows, and shard 1's heap reaches exactly 6 frames before shard 0 seals and never more, all 56 frames emitted. The code is behaviour-identical, so no pre-change red exists; with the frontier mutated so an empty-heap live shard always blocks, the arm reds from window 1 (7 frames held against the bound of 6). `malf test consumer` runs 22 of 22 on clang-21 and 22 of 22 on gcc-16.2. The liveness arm `DN-98.D6` leaves to Kleio (shard 0 stops sealing and the heap grows by 5 a window) was not written.
