...
**Current package baseline:** `coderoast_ipc_core/1.1.0`, `coderoast_ipc_consumer/1.1.0`, and `coderoast_ipc_producer/1.1.0`. Cross-repo compatibility is tracked in [../technical_docs/compatibility_matrix.md](../technical_docs/compatibility_matrix.md), and planning lives in [../technical_docs/ROADMAP.md](../technical_docs/ROADMAP.md).
...
Tags use `v1.1.0` semver format. GitHub Actions automatically:

1. Verify `conanfile.py` version matches tag
2. Build all three packages
3. Export Conan cache tarball (`coderoast_ipc-1.1.0.tgz`)
4. Attach tarball to GitHub release

Consumers restore with:
```bash
conan cache restore coderoast_ipc-1.1.0.tgz
```
